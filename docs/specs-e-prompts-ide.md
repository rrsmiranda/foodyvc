...
- Um módulo por vez, na ordem: M0 → M4 → M2 → M3 → M1 → M5.
...
2. Rode os módulos **na ordem** (M1 → M4 → M2 → M3 → M5). M4 (mock) antes de M2/M3 permite testar sem infra.
...
`M0 (scaffold) → M4 (mock) → M2 (verify_service) → M3 (AgeCheckPage+lifecycle) → M1 (casca compra) → M5 (testes)`
