...
> Última atualização: 2026-09-10 · v10 (prontidão p/ teste real: flags de config + tela de diagnóstico)
...
- **`verifyServiceProvider`** (`Provider`, app-wide) — em teste sobrescrever com `VerifyService(dio: mock.dio, uriLauncher: ...)`. **`ageCheckTimingProvider`** (`AgeCheckTiming { pollInterval=2s, maxPolls=90 }`) — testes zeram o intervalo. **`ageCheckOutcomeProvider`** (`StateProvider<AgeCheckOutcome?>`, NÃO autoDispose) — grava `verified`/`blocked` p/ o gate do M1 ler depois de sair da tela.
